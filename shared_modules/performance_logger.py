...
Log files (all in output_data/<jurisdiction>/performance_logs/):
  log_extract_session.jsonl  - One row per full run (summary)
  log_extract_fetch.jsonl    - One row per ruling document fetch
  log_extract_regex.jsonl    - One row per ruling regex parse
  log_extract_llm.jsonl      - One row per ruling LLM call
"""
...
class PerformanceLogger:
    ...
    def track_fetch(self, ruling_id: str, start: datetime, end: datetime, status: str, cache_hit: bool):
        """Log one document fetch event to log_extract_fetch.jsonl."""
        ...
        self._append("log_extract_fetch.jsonl", entry)

    def track_regex(self, ruling_id: str, start: datetime, end: datetime, status: str):
        """Log one regex parse event to log_extract_regex.jsonl."""
        elapsed = (end - start).total_seconds()
        ...
        self._append("log_extract_regex.jsonl", entry)
    ...
    def track_llm_call(self, provider: str, model: str, input_tokens: int, output_tokens: int,
                       ...
                       status: str = "Complete"):
        """Log one LLM call event to log_extract_llm.jsonl."""
        ...
        self._append("log_extract_llm.jsonl", entry)
    ...
    def write_log(self, jurisdiction: str):
        """Write session summary to log_extract_session.jsonl and print console summary."""
        ...
        self._append("log_extract_session.jsonl", log_entry)

        # Console summary
        session_log_path = os.path.join(self.log_dir, "log_extract_session.jsonl")
